...
"""
    ...
    def show_battery(self, _value: str = "") -> None:
        if IS_MAC:
            ...
            output = self._capture(["upower", "-i", "/org/freedesktop/UPower/devices/battery_BAT0"])
        else:
            output = None
        if output is None:
    ...
    def take_screenshot(self, _value: str = "") -> None:
        ...
        if IS_MAC:
            ...
            success = self._run(["scrot", "-u", str(path)])
            if not success:
                success = self._run(["gnome-screenshot", "-f", str(path)])
        else:
    ...
    def notify(self, message: str) -> None:
        ...
        if IS_MAC:
            ...
            self._run(["notify-send", "Supremo", message])
        else:
    ...
    def copy_to_clipboard(self, text: str) -> None:
        ...
        if IS_MAC:
            ...
            for tool in [["xclip", "-selection", "c"], ["wl-copy"]]:
                try:
                    subprocess.run(tool, input=text, text=True, check=True, timeout=10)
                    print("Copied to the clipboard.")
                    return
                except (OSError, subprocess.TimeoutExpired, subprocess.CalledProcessError):
                    continue
            print("Could not copy to the clipboard: install xclip or wl-clipboard")
            return
        else:
            print(f"(text-to-clipboard not available on this platform: {text[:50]}...)")
            return
        try:
